...
export default class User {
    ...
    save = async () => {
        try {
         const res = await queryer.exec('INSERT INTO alexis_user(first_name, last_name, email, password) VALUES ($1, $2, $3, $4) RETURNING *', ...this.values())
         return User.hydrate(res.row).pop()
        }catch (err){
            return err
        }
    }
...
//falta el cuerpo de la funcion
    }

    static findByEmailAndComparePassword = async (email, password)=>{
        try{
            const res = queryer.exec('SELECT * FROM alexis_user WHERE email = $1', email)
